...
var reduce = window.matchMedia(
        ...
        function render() {
          ...
          var cy =
            centers[prevI] + (centers[nextI] - centers[prevI]) * smoother(tt); // ride with the section, settle with it
          ...
          if (glow > 0.02) {
            ...
            rg.addColorStop(1, "rgba(" + TEAL[0] + "," + TEAL[1] + "," + TEAL[2] + ",0)");
            ctx.fillStyle = rg;
          ...
          if (cloudOp > 0.02) {
            ...
            for (var i = 0; i < proj.length; i++) {
              var q = proj[i];
              var alpha =
                Math.min(1, (0.16 + (q.z + 1) * 0.34) * (1 + glow * 0.4)) *
                mAlpha;
